# Enhanced Full Aggregation Script - Updated for 57 weeks (2024-09-02 to 2025-09-29)
"""
...
**Scope:** Full {len(weeks)}-week dataset  
...
**Expected Rows:** {len(weeks) * 3}  
"""

    expected_rows = len(weeks) * 3
    if len(rows) == expected_rows:
        report += "**Status:** ✅ MATCH\n"
    else:
        report += (
            f"**Status:** ❌ MISMATCH (expected {expected_rows}, got {len(rows)})\n"
        )
...
### Completeness
- **Expected rows:** {len(weeks) * 3} (3 platforms × {len(weeks)} weeks)
...
"""
    ...
    status_emoji = "✅" if len(rows) == expected_rows else "❌"
    report += f"- Row count: {len(rows)}/{expected_rows} {status_emoji}\n"
...
def main():
    ...
    csv_path = OUTDIR / "social_platform_usage_weekly_2024-09-02_to_2025-09-29.csv"
    xlsx_path = OUTDIR / "social_platform_usage_weekly_2024-09-02_to_2025-09-29.xlsx"
    ...
    status = "✅ MATCH" if len(rows) == expected_rows else "❌ MISMATCH"
